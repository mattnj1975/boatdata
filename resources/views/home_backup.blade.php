                    ...
                    <form id="searchForm" action="{{ route('search.mac') }}" method="post">
                        ...
                        <div class="row">
                            <div class="col-lg-10 col-md-10 col-sm-6">
                                <input required type="text" name="mac" class="form-control" placeholder="Search by mac or by email address ... ">
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('searchForm').addEventListener('submit', function(event) {
            ...
            document.getElementsByClassName('boat_details').innerHTML = '';
            ...
            fetch(this.action, {
                ...
                .then(data => {
                    if (Array.isArray(data) && data.length > 0) {
                        var tableHTML = '<table class="table table-responsive"><thead><tr><th>Boat Name</th><th>MAC</th><th>Trip Date</th><th>Start</th><th>Finish</th><th>Duration</th><th>Distance</th><th></th></tr></thead><tbody>';
                        data.forEach(function(item) {
                            ...
                            tableHTML += '<tr>';
                            tableHTML += '<td>' + item.boatname + '</td>';
                            tableHTML += '<td data-mac="' + item.mac + '">' + item.mac + '</td>';
                        ...
                        var tripDate = new Date(data.TripDate);
                        var formattedTripDate = ('0' + tripDate.getDate()).slice(-2) + '/' + ('0' + (tripDate.getMonth() + 1)).slice(-2) + '/' + tripDate.getFullYear();
                        var tableHTML = '<table class="table table-responsive"><thead><tr><th>Boat Name</th><th>MAC</th><th>Trip Date</th><th>Start</th><th>Finish</th><th>Duration</th><th>Distance</th><th></th></tr></thead><tbody>';
                        tableHTML += '<tr>';
                        tableHTML += '<td>' + data.boatname + '</td>';
                        tableHTML += '<td data-mac="' + data.mac + '">' + data.mac + '</td>';
                        tableHTML += '<td data-date="' + formattedTripDate + '">' + formattedTripDate + '</td>';
                        tableHTML += '<td>' + (data.Begin ? data.Begin : 'N/A') + '</td>';
                        tableHTML += '<td>' + (data.Finish ? data.Finish : 'N/A') + '</td>';

                        if (data.Begin && data.Finish) {
                            var beginTime = new Date(data.TripDate + ' ' + data.Begin);
                            var finishTime = new Date(data.TripDate + ' ' + data.Finish);
                            var durationInMinutes = Math.round((finishTime - beginTime) / (1000 * 60));
                            var hours = Math.floor(durationInMinutes / 60);
                            var minutes = durationInMinutes % 60;
                            var duration = hours + 'h ' + minutes + 'm';
                        ...
                        tableHTML += '<td>' + (data.Trip ? data.Trip : 'N/A') + '</td>';
                        tableHTML += '<td><div class="d-flex btn-group-lg" role="group" aria-label="Basic example"><i style="cursor: pointer;" class="fa fa-solid fa-map viewLink viewTrack" title="Track"></i>&nbsp;&nbsp;&nbsp;<i style="cursor: pointer;" class="fa fa-solid fa-table viewLink viewData" title="Data"></i></div></td>';
                        tableHTML += '</tr>';
                        tableHTML += '</tbody></table>';
                        document.getElementById('searchResults').innerHTML = tableHTML;
                    } else {
                        document.getElementById('searchResults').innerHTML = 'No record found.';
                    }
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", ".viewTrack", function() {
            ...
            let mac = $(this).closest('tr').find('td[data-mac]').data('mac');
        ...
        function loadLog() {
            ...
            if ($("body").is("[data-loading]")) {
                ...
                $.post("/view/assets/ajax/get-log-data", {
                    date: date,
                    mac: mac
                }, function(data) {
                    if ($("#log-tab-pane .dataTable").length > 0) {
                        dataTable.destroy()
                    }
                    console.log(data)
        ...
        function loadTable() {
            if ($("body").is("[data-loading]")) {
                ...
                $.post("{{route('admin.getTableData')}}", {
                    ...
                    if ($("#data-tab-pane .dataTable").length > 0) {
